...
- Deliberate travel to a control the action just opened (writing form, joke picker, show dialog) now goes through `scrollControlIntoView()`, which bumps a control id so the rest of that beat's focus passes are void — the beat can no longer drag the player away from what they opened, while the next beat gets a clean claim. `hideDialog`, the last `dismissCriticalDialog` and the show result beat hand the viewport back, and the ending forces it (`sceneFocusAllowed()` in script.js holds the rule).
- New `scripts/verify-scene-focus.js` (CDP, re-runnable like `verify-intro-gate.js`): boots a run, parks the viewport as far from the scene as the document allows, and asserts a read beat re-centers the scene, the writing form and joke picker keep the viewport, dialogs stay readable, finishing a joke / booking a show / the show result return to the scene. PASSES on a local server and against the live site at 1366×768, 1440×900, 1920×1080 and mobile-emulated 412×915.
- Note on the desktop geometry: at ≥1200px wide the scene image is 560px tall, so image + a 2-line message is taller than a 768px window; the focus top-aligns the section there and the last line can sit just below the fold. Shrinking that image (e.g. 560 → 420) would let the whole section fit — not done, pending Illan's call.
